...
An installable Codex plugin, containing the repo's bounded `ledger-contribution-pack` Skill, creates a versioned Contribution Pack from evidence explicitly selected by the user. Ledger validates the pack deterministically, previews editable claims, and accepts selected claims into `pending_review`. A no-API-key Demo PM Agent records an advisory progress-evidence assessment. Another authenticated member still performs peer confirmation, and Postgres remains responsible for authorization, append-only behavior, idempotent import, and canonical Evidence Hash v3.
...
- `Agent Verified` is advisory evidence pre-verification and never changes human review fields.
